...
const totalResult = computed(() => {
  return props.uproductions.data
    .reduce(
      (total, item) =>
        total +
        item.uincrements.reduce(
          (sum, item) => sum + item.price * item.quantity,
          0
        ) -
        item.udecrements.reduce(
          (sum, item) => sum + item.price * item.quantity,
          0
        ),
      0
...
const uproductionsIndex = (requestProp) => {
  router.get(
        ...
        <div class="body-panel">
          <div class="scrollable-content">
            <q-table
              ...
              bordered
              :title="
                uhall_name ? `Процеси Угояване в Хале: ${uhall_name}` : title
              "
                ...
                >
                  ...
                  >
                    ...
                    >
                      {{
                        (
                          props.row.uincrements.reduce(
                            (sum, item) => sum + item.price * item.quantity,
                            0
                          ) -
                          props.row.udecrements.reduce(
                            (sum, item) => sum + item.price * item.quantity,
                            0
                          )
                        ).toFixed(2)
                    ...
                    >
                      ...
                      <q-btn
                        v-if="
                          ...
                          (props.row.status === 0 ||
                            (props.row.product === null &&
                              props.row.udecrements.length === 0 &&
                              props.row.uincrements.length === 0))
                        "
