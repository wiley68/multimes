...
const productionPurcent = (mproduction) => {
    ...
    return parseFloat(
        (days / parseFloat(mproduction.production_days)).toFixed(2)
    );
};
const productionPurcentLabel = (mproduction) => {
    return `${(productionPurcent(mproduction) * 100).toFixed(2)}%`;
};
...
const confirm = (mhall) => {
    $q.dialog({
        title: "Потвърди",
        message:
            "Ще бъде стартиран нов производствен процес! Процеса е необратим. Съгласен ли сте с това?",
        cancel: true,
        persistent: true,
        ok: {
            label: "Да",
