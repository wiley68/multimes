...
const mincrementsComplete = (mincrement) => {
  $q.dialog({
    title: 'Потвърди',
    message:
      'Желаеш ли да приключиш този приход? Количеството от избрания продукт ще бъде намалено в халето за текущия процес, като ще промени текущите наличности. Процеса е необратим!',
    cancel: true,
    persistent: true,
    ok: {
      label: 'Да',
              ...
              <div v-else="col.name === 'actions'" style="width: 60px">
                <template v-if="mproduction.mhall.type === 'Родилно'">
                  <q-btn
                    ...
                    rounded
                    @click="
                      mincrementsCompletePodrastvane({
                        id: props.row.id,
